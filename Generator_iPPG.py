...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
            ...
            for subdir in sorted(os.listdir(directory)):
                if os.path.isdir(os.path.join(directory, subdir)):
                    ...
                    for task in sorted(os.listdir(tasks)):
                        ...
                        cls_emo.append(os.path.splitext(os.path.normpath(cls))[0][-1:].replace("3", "2"))
                        vid.append(task)
                        classes.append(cls)
                        list_im = sorted(os.listdir(cls))
        self.num_class = len(vid)
        self.class_emotions = dict(zip(cls_emo, range(len(classes))))

        self.class_indices = dict(zip(classes, range(len(classes))))
        print(self.class_emotions)
        ...
        for ti in classes_hr:
            ...
            heart_rate = [filename for filename in list_dir2 if filename.startswith("ippg")]
            for hr in heart_rate:
                Heart_rate_resampled = os.path.join(ti + '/' + hr)
                # df = pd.read_csv(Heart_rate_resampled, index_col=0)
                df = np.genfromtxt(Heart_rate_resampled, delimiter='\n')
                batches_hr.append(df)
        li = []
        Heart_Rate = []
    ...
    def next(self):
        ...
        #                    self.image_shape)  # # my addition of +(1,)        #print(batch_x.shape)
        ...
        if self.class_mode == 'input':
            ...
            for j in range(self.batch_size):
                for k in range(len(list(self.class_emotions.keys()))):
                    ...
                    classes_emo = os.path.splitext(os.path.normpath(fname))[0][-2:]
...
if __name__ == '__main__':
    ...
    train_data = datagen.flow_from_directory(directory='/media/bousefsa1/My Passport/BD PPG/2 bases publiques/ubfc_phys_frames',
                                              label_dir='/media/bousefsa1/My Passport/BD PPG/2 bases publiques/ubfc_phys_ippg',
                                             target_size=(1024, 1024), class_mode='PPG', batch_size=1,
                                         frames_per_step = 6300, shuffle=False)
